...
function init_state()
  ...
  bcs = (
    ilo=DirichletBC(1.0125),  #
    ihi=DirichletBC(-0.0125),  #
    jlo=NeumannBC(),  #
    jhi=NeumannBC(),  #
  )
...
begin
  ...
  T1d = copy(T[:, 1])
  T_analytic = reverse(x) |> collect
  err = norm(T1d - T_analytic)
  @show err
  plot(xc[:, 1], T[:, 1]; label="simulation", marker=:circle)
  plot!(x, reverse(x); label="analytic", color=:black, ls=:dash)
end
