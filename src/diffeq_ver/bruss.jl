...
function brusselator_2d_loop(du, u, p, t)
  ...
  @inbounds for I in CartesianIndices((N, N))
    ...
    du[i, j, 1] =
      alpha * (u[im1, j, 1] + u[ip1, j, 1] + u[i, jp1, 1] + u[i, jm1, 1] - 4u[i, j, 1]) +
      B +
      u[i, j, 1]^2 * u[i, j, 2] - (A + 1) * u[i, j, 1] + brusselator_f(x, y, t)
    du[i, j, 2] =
      alpha * (u[im1, j, 2] + u[ip1, j, 2] + u[i, jp1, 2] + u[i, jm1, 2] - 4u[i, j, 2]) +
...
jac_sparsity = Symbolics.jacobian_sparsity(
  (du, u) -> brusselator_2d_loop(du, u, p, 0.0), du0, u0
)
...
# @profview solve(prob_ode_brusselator_2d, TRBDF2(), save_everystep=false);
# @profview solve(prob_ode_brusselator_2d_sparse, TRBDF2(), save_everystep=false);
@profview solve(
