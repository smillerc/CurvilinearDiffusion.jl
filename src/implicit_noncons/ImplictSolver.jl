...
function ImplicitScheme(
  ...
  if backend isa CPU
    ...
    A_cache = init_A_matrix(mesh, CPU())
    Pl = kp_ilu0(A)
  end
...
function solve!(
  ...
  if scheme.backend isa CPU
    _cpu_solve!(scheme, mesh, u, Δt; maxiter=maxiter, show_hist=show_hist)
  else
    _gpu_solve!(scheme, mesh, u, Δt; maxiter=maxiter, show_hist=show_hist)
  end
...
function _gpu_solve!(
  ...
  # update the A matrix
  @timeit "assemble_matrix" assemble_matrix!(scheme, scheme._A_cache, mesh, u, Δt)
  scheme.A .= CuSparseMatrixCSR(scheme._A_cache)

  # precondition
  @timeit "ilu0! (preconditioner)" update!(scheme.Pl, scheme.A)
