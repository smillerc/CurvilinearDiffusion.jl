...
# Fields
 ...
 - `nranks`: Number of total processors (global)
 - `rank`: Current rank
...
"""
struct CartesianTopology{NB} <: AbstractParallelTopology
  comm::MPI.Comm
  nranks::Int
  rank::Int
  dimension::Int
  ...
  neighbors::NB   # [[ilo, center, ihi], i, j, k]; defaults to -1 if no neighbor
end
...
```
"""
function CartesianTopology(
  comm::MPI.Comm, dims::NTuple{N,Int}, periodicity::NTuple{N,Bool}; canreorder=false
) where {N}
  dims = collect(dims)
  ...
  @assert prod(dims) == nranks "The number of subdomains ($(prod(dims))) doesn't match the number of procs ($(nranks)) provided"
  ...
  neighbors = OffsetArray(
    Int(MPI.API.MPI_PROC_NULL[]) * ones(Int8, 3, 3, 3), -1:1, -1:1, -1:1
  )
...
function CartesianTopology(
  comm::MPI.Comm, dims::Vector{Int}, periodicity::Vector{Bool}; canreorder=false
)
...
"""
function CartesianTopology(comm::MPI.Comm, nranks::Int, periodicity::Bool; canreorder=false)
  CartesianTopology(comm, nranks, tuple(periodicity); canreorder=canreorder)
end
...
"""
function CartesianTopology(
  comm::MPI.Comm, nranks::Int, periodicity::NTuple{N,Bool}; canreorder=false
) where {N}
  if length(periodicity) == 3
    dims = num_3d_tiles(nranks)
  elseif length(periodicity) == 2
    dims = num_2d_tiles(nranks)
...
function offset_coord_to_rank(
  comm, dims, periods, i_offset::Int, j_offset::Int, k_offset::Int
)
...
function Base.show(io::IO, p::AbstractParallelTopology)
  ...
  println(io, "rank $(p.rank) of $(p.nranks)")
...
function on_boundary(p::CartesianTopology, loc::Symbol)
  locations = (;
    ...
    jlo=jlo_neighbor(p) == Int(MPI.API.MPI_PROC_NULL[]),
    jhi=jhi_neighbor(p) == Int(MPI.API.MPI_PROC_NULL[]),
    klo=klo_neighbor(p) == Int(MPI.API.MPI_PROC_NULL[]),
    khi=khi_neighbor(p) == Int(MPI.API.MPI_PROC_NULL[]),
  )
...
function global_to_subdomain_bounds(
  globalarraysize::NTuple{1,T}, topology::CartesianTopology, p
) where {T<:Integer}
  ilo, ihi = tile_indices_1d1(
    globalarraysize[1], topology.global_dims[1], topology.coords[1]
  )
  return (ilo, ihi)
end

function global_to_subdomain_bounds(
  globalarraysize::NTuple{2,T}, topology::CartesianTopology, p
) where {T<:Integer}
  ...
  jlo, jhi = tile_indices_1d1(
    globalarraysize[2], topology.global_dims[2], topology.coords[2]
  )
...
function global_to_subdomain_bounds(
  globalarraysize::NTuple{3,T}, topology::CartesianTopology, p
) where {T<:Integer}
  ilo, ihi = tile_indices_1d1(
    globalarraysize[1], topology.global_dims[1], topology.coords[1]
  )
  ...
  klo, khi = tile_indices_1d1(
    globalarraysize[3], topology.global_dims[3], topology.coords[3]
  )
