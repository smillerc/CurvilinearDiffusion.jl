...
function flux_divergence(
  ...
  @inbounds begin
    ...
    # ∂(qᵢ)/∂ξ - orth:  (ξx² + ξy²) (qᵢ[i, j] - qᵢ[i-1, j])
    # ∂(qⱼ)/∂ξ - cross: (ηx ξx + ηy ξy)

    # ∂(qⱼ)/∂η - orth: (ηx² + ηy²) (qⱼ[i, j] - qⱼ[i, j-1])
    # ∂(qᵢ)/∂η - cross

    # ηξ_1 = (
    #   cell_center_metrics.ξ.x₁[i, j + 1] * cell_center_metrics.η.x₁[i, j + 1] +
    #   cell_center_metrics.ξ.x₂[i, j + 1] * cell_center_metrics.η.x₂[i, j + 1]
    # )
    # ηξ_2 = (
    #   cell_center_metrics.ξ.x₁[i, j - 1] * cell_center_metrics.η.x₁[i, j - 1] +
    ...
    # ∂qᵢ∂η =
    #   0.5(
    #     ηξ_1 * (qᵢ[i, j + 1] - qᵢ[i - 1, j + 1]) - # take average on either side
    #     ηξ_2 * (qᵢ[i, j - 1] - qᵢ[i - 1, j - 1])   # and do diff in j
    #   )

    # ηξ_3 = (
    #   cell_center_metrics.ξ.x₁[i + 1, j] * cell_center_metrics.η.x₁[i + 1, j] +
    #   cell_center_metrics.ξ.x₂[i + 1, j] * cell_center_metrics.η.x₂[i + 1, j]
    # )
    # ηξ_4 = (
    #   cell_center_metrics.ξ.x₁[i - 1, j] * cell_center_metrics.η.x₁[i - 1, j] +
    ...
    # ∂qⱼ∂ξ =
    #   0.5(
    #     ηξ_3 * (qⱼ[i + 1, j] - qⱼ[i + 1, j - 1]) - # take average on either side
    #     ηξ_4 * (qⱼ[i - 1, j] - qⱼ[i - 1, j - 1])   # and do diff in i
    #   )
    ∂qᵢ∂η =
      0.25(ηx * ξx + ηy * ξy) * (
